...
do-dev-setup:
	git submodule init $(BENCH_DESIGN_HOME)/src/$(DEV_DESIGN_HOME)/dev/repo
	git submodule update $(BENCH_DESIGN_HOME)/src/$(DEV_DESIGN_HOME)/dev/repo
    # Check if a setup.sh script exists for the current design
	@if [ -f "$(BENCH_DESIGN_HOME)/src/$(DEV_DESIGN_HOME)/dev/setup.sh" ]; then \
		bash $(BENCH_DESIGN_HOME)/src/$(DEV_DESIGN_HOME)/dev/setup.sh; \
	fi
