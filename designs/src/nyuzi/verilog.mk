...
TARGET_DEV_FILE = $(BENCH_DESIGN_HOME)/src/$(DESIGN_NAME)/dev/nyuziTop.v

$(TARGET_DEV_FILE) : $(REPO_FILES) 
    # Bypass error if patch has already been applied (prone to fail if repo code has changed)
	patch -p0 -N --silent --directory=$(REPO_SRC_DIR) < $(BENCH_DESIGN_HOME)/src/$(DESIGN_NAME)/patch-all.patch > /dev/null 2>&1 || [[ $$? == 1 ]]
	$(BENCH_DESIGN_HOME)/src/$(DESIGN_NAME)/dev/sv2v --top nyuzi -w $@ -I $(REPO_INCLUDE_FILES) $(REPO_FILES)

export VERILOG_FILES = $(TARGET_DEV_FILE) \
                       $(BENCH_DESIGN_HOME)/$(PLATFORM)/$(DESIGN_NAME)/macros.v
                       
else
export VERILOG_FILES = $(BENCH_DESIGN_HOME)/src/$(DESIGN_NAME)/nyuziTop.v \
                       $(BENCH_DESIGN_HOME)/$(PLATFORM)/$(DESIGN_NAME)/macros.v
endif